...
const Orders = () => {
  ...
  return (
    <>
      {orders.length >= 1 ? (
        <div className="orders-wrapper bg-white xs:m-1.5 rounded-md p-1">
          <h1 className="text-center">Orders</h1>
          <div className="orders-list xs:p-0">
            {orders.map((item, idx) => (
              <div className="ordered-item bg-red-200 border border-red-200 shadow-md shadow-red-400 rounded-md flex xs:flex-col justify-start xs:items-start  xs:p-.5 m-1.5 lg:p-1.5 md:flex-row lg:items-center">
                <div className="image-container xs:p-1">
                  <img
                    src={item.images[0]}
                    alt=""
                    className="rounded-md border border-white shadow-sm shadow-white xs:h-[100%] lg:h-24"
                  />
                </div>
